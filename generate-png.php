...
$nis = (isset($options['nis']) ? explode(',', $options['nis']) : []);
...
foreach ($mapfiles as $i => $mapfile) {
    ...
    if (preg_match('/^([0-9]{5}) - (.+?) - (.+?)$/', $fname, $matches) === 1) {
        ...
        $name_fr = $matches[2];
        $name_nl = $matches[3];
    } elseif (preg_match('/^([0-9]{5}) - (.+?)$/', $fname, $matches) === 1) {
        $municipality = true;
    ...
    list($w, $h) = getimagesize($directory.'/result/'.$fname.'-src.png');
    $w += 4;
    $h += 4;
    ...
    $im1 = imagecreatefrompng($directory.'/result/'.$fname.'-src.png');
    $h1 = imagesy($im1);
    $w1 = imagesx($im1);
    imagecopymerge($im, $im1, 2, 2, 0, 0, $w1, $h1, 100);
    imagedestroy($im1);

    $im2 = imagecreatefrompng($directory.'/result/'.$fname.'-scale.png');
    $h2 = imagesy($im2);
    $w2 = imagesx($im2);
